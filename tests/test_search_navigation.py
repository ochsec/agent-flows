...
class TestSearchNavigation(unittest.TestCase):
    ...
    def test_task_basic_execution(self):
        ...
        result = self.search_nav.task("search files", "Find all Python files in the project")
        ...
        self.assertIn("Python files", result['response'])
        
    def test_task_validation_short_description(self):
        """Test task validation with short description."""
        with self.assertRaises(ValueError) as cm:
            self.search_nav.task("hi", "Long prompt here with enough characters")
    ...
    def test_task_validation_short_prompt(self):
        ...
        with self.assertRaises(ValueError) as cm:
            self.search_nav.task("valid desc", "short")
    ...
    def test_task_search_simulation(self):
        ...
        result = self.search_nav.task("find config", "Find configuration files in the project")
        
        response = result['response']
        self.assertIn("Search Results", response)
        self.assertIn("find configuration files", response.lower())
    ...
    def test_task_implementation_simulation(self):
        ...
        result = self.search_nav.task("implement feature", "Create a new authentication system")
        
        response = result['response']
        self.assertIn("Implementation Analysis", response)
        self.assertIn("authentication system", response.lower())
        self.assertIn("simulated response", response)
        
    def test_task_analysis_simulation(self):
        """Test task simulation for analysis tasks."""
        result = self.search_nav.task("analyze code", "Analyze the current database schema")
        
        response = result['response']
        self.assertIn("Analysis Report", response)
        self.assertIn("database schema", response.lower())
        self.assertIn("simulated response", response)
        
    def test_task_general_simulation(self):
        """Test task simulation for general tasks."""
        result = self.search_nav.task("help debug", "Debug the failing test cases")
        
        response = result['response']
        self.assertIn("Task Results", response)
        self.assertIn("debug the failing test cases", response.lower())
        self.assertIn("simulated response", response)
        
    def test_task_agent_id_generation(self):
        """Test that agent IDs are generated consistently."""
        result1 = self.search_nav.task("test", "Same prompt here")
        result2 = self.search_nav.task("test", "Same prompt here")
        
        # Same prompt should generate same agent ID
        self.assertEqual(result1['agent_id'], result2['agent_id'])
        
        result3 = self.search_nav.task("test", "Different prompt here")
